"""Check or create NoCoBase fields required by user-skill tests.
...
COLLECTION_NAME = "hermes_user_skills"
REQUIRED_FIELDS = (
...
def _field_payloads() -> dict[str, dict[str, Any]]:
    return {
        "security_test_result": {
            ...
            "description": "Hermes WebUI Skill local security scan latest JSON result",
        },
        "security_tested_at": {
            ...
            "description": "Hermes WebUI Skill local security scan latest completion time",
        },
        "availability_test_result": {
            ...
            "description": "Hermes WebUI Skill Promptfoo latest JSON result",
        },
        "availability_tested_at": {
            ...
            "description": "Hermes WebUI Skill Promptfoo latest completion time",
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check or create hermes_user_skills fields required by user-skill "
    ...
    try:
        ...
        final_existing = _list_field_names()
        remaining = [field_name for field_name in REQUIRED_FIELDS if field_name not in final_existing]
        if remaining:
            print("Fields still missing after apply:")
            for field_name in remaining:
                print(f"  - {field_name}")
            return 2
        print("All required user-skill test fields exist.")
        return 0
