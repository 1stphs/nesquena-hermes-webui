...
from api.core.workspace import get_last_workspace
from api.features.chat.agent_sessions import read_importable_agent_session_rows, read_session_lineage_metadata
...
def _get_profile_home(profile) -> Path:
    ...
    Prefers the profile-specific helper from api.core.profiles; falls back to the
...
def new_session(workspace=None, model=None, profile=None, model_provider=None, project_id=None):
    ...
    *profile* — when supplied by the caller (e.g. from the request body sent
    ...
    if profile is None:
        ...
        try:
            from api.core.profiles import get_active_profile_name
...
def ensure_cron_project() -> str:
    ...
    """
    from api.core.profiles import get_active_profile_name, _is_root_profile
