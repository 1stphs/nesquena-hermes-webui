...
# ── Basic layout ──────────────────────────────────────────────────────────────
HOME = Path.home()
# REPO_ROOT is the repository root above api/.
...
def _discover_agent_dir() -> Path:
    ...
    # 4. Repo-local hermes-agent-src checkout used by this workspace.
    candidates.append(REPO_ROOT / "hermes-agent-src")
...
def _current_webui_version() -> str | None:
    ...
    `api.features.misc.updates` imports `api.core.config` at module-load time, so we cannot
    `from api.features.misc.updates import WEBUI_VERSION` at the top of this module without a
    circular import. Instead we resolve lazily on each cache load/save.

    Returns the runtime version string (e.g. ``v0.50.293``) when api.features.misc.updates
    ...
    try:
        ...
        import sys as _sys
        mod = _sys.modules.get('api.features.misc.updates')
        if mod is None:
            return None
...
def _get_auth_store_path() -> Path:
    ...
    try:
        from api.core.profiles import get_active_hermes_home as _gah
...
def _save_models_cache_to_disk(cache: dict) -> None:
    ...
    api.features.misc.updates is loaded. _is_loadable_disk_cache treats a missing field as
...
def get_available_models() -> dict:
    ...
    def _build_available_models_uncached() -> dict:
        ...
        if not _hermes_auth_used:
            try:
                from api.core.profiles import get_active_hermes_home as _gah2
...
def save_settings(settings: dict) -> dict:
    ...
    if raw_pw and isinstance(raw_pw, str) and raw_pw.strip():
        ...
        from api.authn.auth import _hash_password
...
try:
    from api.core.profiles import init_profile_state
