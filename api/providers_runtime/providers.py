...
def _write_env_file(env_path: Path, updates: dict[str, str | None]) -> None:
    ...
    Holds ``_ENV_LOCK`` from ``api.runtime.streaming`` for the entire load → modify →
    ...
    """
    from api.runtime.streaming import _ENV_LOCK
...
def _fetch_account_usage_with_profile_context(provider: str) -> Any:
    try:
        from api.core.profiles import cron_profile_context_for_home
    except ImportError:
    ...
    for pid in sorted(known_ids):
        ...
        if pid == "nous":
            try:
                ...
                if live_ids:
                    # Lazy-import to avoid circular dep with api.core.config.
                    from api.core.config import _format_nous_label, _build_nous_featured_set
...
def _clean_provider_key_from_config(provider_id: str) -> None:
    ...
    """
    from api.core.config import _cfg_lock

    try:
        # Resolve through api.core.config at call time instead of the function imported
        # at module load. Several tests (and some profile flows) monkeypatch the
        # config module's path resolver after api.providers_runtime.providers has already been
        # imported; using the stale imported reference can clean the wrong
        # config.yaml.
        import api.core.config as _config
