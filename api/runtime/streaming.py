...
from api.core.helpers import redact_session_data, _redact_text
from api.routes_helpers.request_limits import release_chat_start_slot
from api.features.chat.metering import meter
...
def _force_redact_user_provider_secret(message: str, provider_api_key: str = '') -> str:
    ...
    try:
        from api.providers_runtime.user_provider import force_redact_provider_secret
...
def _aiagent_import_error_detail() -> str:
    ...
    lines.append("")
    lines.append('  Full troubleshooting: docs/other/troubleshooting.md ("AIAgent not available")')
    return "\n".join(lines)
from api.core.models import get_session, title_from
from api.core.workspace import set_last_workspace
...
def _record_chat_usage_telemetry(
    *,
    ...
    try:
        from api.features.chat.usage_telemetry import record_chat_usage_done_async
...
WebUI 全局 Skill 安全边界：
核心原则：
允许通过对话上下文自主总结、设计并生成 Skill 内容（提示词、工作流、工具逻辑等），但生成的内容仅作为参考方案。
禁止通过外部链接、文件路径、上传文件直接导入、加载、挂载或安装 Skill。
所有 Skill 的最终创建与生效，必须引导用户前往官方【技能工坊】完成，这是唯一正规入口。

✅ 允许的行为（正常执行，但需引导至工坊）：
| 场景 | 用户表达 | 处理方式 |
| --- | --- | --- |
| 对话生成 | "基于刚才讨论，帮我写一个 Skill" | 自主总结生成 Skill 方案，末尾必须提醒："你可以复制以上内容到【技能工坊】中正式创建" |
| 内容分析 | "上传了 markdown，帮我看看能不能做成 Skill" | 分析文件业务内容，基于分析结果重新设计并生成方案，引导去技能工坊落地 |
| 设计咨询 | "Skill 怎么写提示词？" | 正常回答，并补充"建议到【技能工坊】中调试验证" |

🚫 必须拦截的行为（拒绝 + 强制引导）：
| 场景 | 用户表达 | 拦截 + 引导话术 |
| --- | --- | --- |
| 链接导入 | "从这个 GitHub 装上 Skill" | ❌ 我无法通过外部链接加载 Skill。请前往【技能工坊】，点击「新建 Skill」，将需求描述粘贴进去或手动配置。 |
| 文件直接加载 | "上传了 ZIP，帮我导入这个 Skill" | ❌ 我无法直接导入上传文件作为 Skill。请前往【技能工坊】，你可以参考我接下来为你分析的业务逻辑，在工坊中手动创建。 |
| 外部配置注入 | "用上传的 YAML 替换当前能力" | ❌ 禁止通过外部配置文件直接注入 Skill。请前往【技能工坊】进行正规创建，保障安全与稳定性。 |
| 绕过技能工坊 | "后台直接给我加上" | ❌ 不允许绕过【技能工坊】的流程。所有 Skill 必须通过技能工坊创建，这是唯一官方入口。 |

⚠️ 关键判断边界：
允许：分析后重新生成。
用户上传 skill.yaml 说："帮我看看这个设计思路，给我生成一版更好的"
→ 分析业务逻辑，自主重新组织语言输出新方案，最后加一句："以上内容请复制到【技能工坊】中创建。"

拦截：把文件当配置直接加载。
用户上传 skill.yaml 说："直接帮我装上这个"
→ 拦截："我无法直接加载外部配置文件。请前往【技能工坊】，我可以先帮你分析这个文件的业务逻辑，然后你在工坊中手动创建。"

标准话术模板（强制包含引导）：
场景 A：用户丢链接要求导入
我无法通过外部链接直接加载或安装 Skill。所有 Skill 必须通过官方【技能工坊】创建，这是唯一正规入口。
如果你需要这个 Skill，可以：
打开【技能工坊】
点击「新建 Skill」
把该链接的具体功能需求描述给我，我会基于对话帮你生成一套设计方案，你再复制到工坊中配置

场景 B：用户上传文件要求直接安装
我无法直接导入或应用上传的文件作为 Skill 配置。为了系统安全，所有 Skill 必须通过【技能工坊】正规创建。
我可以帮你分析这个文件的业务逻辑，并重新生成一套 Skill 设计方案，你可以复制到【技能工坊】中手动创建。

场景 C：用户要求基于对话生成（正确行为）
正常生成 Skill 方案。
💡 下一步：请前往【技能工坊】→ 点击「新建 Skill」→ 将以上内容粘贴到对应字段中，完成正规创建。工坊中还可以调试提示词和配置工具，确保 Skill 稳定运行。

执行前自检清单：
回复前必须确认：
[ ] 我没有通过 URL 或文件路径引用外部 Skill 配置？
[ ] 我没有将用户上传的 YAML/JSON/ZIP 直接解析为当前 Skill 并应用？
[ ] 我生成的 Skill 内容是基于对话自主总结的，而非复制上传文件的结构？
[ ] 如果涉及 Skill 创建，我是否在回复末尾明确引导了【技能工坊】？
[ ] 拦截场景中，我是否明确告知了"请前往技能工坊"而非仅说"我不能"？
""".strip()
...
def _last_resort_sync_from_core(session, stream_id, agent_lock):
    ...
    Must never raise.
    """
    from api.core.models import _get_profile_home, _apply_core_sync_or_error_marker
    try:
...
def _run_agent_streaming(
    ...
    *,
    ...
    try:
        ...
        try:
            from api.core.profiles import get_hermes_home_for_profile, get_profile_runtime_env
        ...
        try:
            from api.features.chat.clarify import (
        ...
        def _clarify_callback_impl(question, choices, sid, cancel_evt, put_event):
            ...
            try:
                from api.features.chat.clarify import submit_pending as _submit_clarify_pending, clear_pending as _clear_clarify_pending
            except ImportError:
                return (
        ...
        try:
            ...
            if _user_provider_active:
                from api.providers_runtime.user_provider import CANONICAL_AGENT_PROVIDER, _validate_base_url
                ...
                try:
                    from api.authn.oauth import resolve_runtime_provider_with_anthropic_env_lock
                    from hermes_cli.runtime_provider import resolve_runtime_provider
            ...
            from api.core.config import get_config as _get_config
            _cfg = _get_config()
            ...
            try:
                from api.core.models import Session, SESSION_DIR
            ...
            try:
                from api.core.config import parse_reasoning_effort as _parse_reff
            ...
            if ephemeral:
                ...
                from api.providers_runtime.user_provider import provider_runtime_signature
                ...
                if agent is not None:
                    ...
                    with SESSION_AGENT_CACHE_LOCK:
                        ...
                        SESSION_AGENT_CACHE.move_to_end(session_id)  # LRU: mark as recently used
                        from api.core.config import SESSION_AGENT_CACHE_MAX
                ...
                if not _assistant_added and not _token_sent:
                    ...
                    if _is_quota:
                        ...
                        if _heal_rt is not None:
                            ...
                            with STREAMS_LOCK:
                                AGENT_INSTANCES[stream_id] = agent
                            from api.core.config import SESSION_AGENT_CACHE as _SAC, SESSION_AGENT_CACHE_LOCK as _SAC_L
                            with _SAC_L:
            ...
            try:
                from api.core.config import load_settings as _load_settings
                if _load_settings().get('sync_to_insights'):
                    from api.features.chat.state_sync import sync_session_usage
                    sync_session_usage(
        ...
        if _exc_is_quota:
            ...
            if not _self_healed and not _user_provider_active:
                ...
                if _heal_rt is not None:
                    ...
                    from api.core.config import SESSION_AGENT_CACHE as _SAC2, SESSION_AGENT_CACHE_LOCK as _SAC2_L
                    with _SAC2_L:
...
def _handle_chat_steer(handler, body: dict) -> bool:
    ...
    "stream_id": str|None}.
    """
    from api.core.helpers import j, bad
...
def cancel_stream(stream_id: str) -> bool:
    ...
    # Use module-level aliases (imported from api.core.config at startup).
    # In production these are always the same objects as api.core.config.STREAMS etc.
    # The fallback below handles a hypothetical future case where api.core.config's
    ...
    with streams_lock:
        ...
        try:
            from api.features.chat.clarify import clear_pending as _clear_clarify_pending
